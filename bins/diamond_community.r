...
opt_parser = OptionParser(
  usage = "usage: gcompip custom diy [options]",
...
tmp$GAM <- as.numeric(tmp$GAM)
tmp$RPKM <- as.numeric(tmp$RPKM)
...
ggsave(plot = hp,"Heatmap_GAM.pdf",width = w*2,height = h/4,limitsize = F)
print("All Completed!")
